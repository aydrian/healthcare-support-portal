...
from .models import User, Patient, Document, Embedding
        ...
        try:
            with oso.batch() as tx:
                ...
                for patient in patients:
                    tx.insert(("has_role", user_value, "admin", Value("Patient", str(patient.id))))
                
                # Admin access to all documents  
                documents = db.query(Document).all()
                for doc in documents:
                    tx.insert(("has_role", user_value, "admin", Value("Document", str(doc.id))))
                
                # Admin access to all embeddings
                embeddings = db.query(Embedding).all()
                for embed in embeddings:
                    tx.insert(("has_role", user_value, "admin", Value("Embedding", str(embed.id))))
                
                # Admin access to all users
                users = db.query(User).filter(User.is_active).all()
                for other_user in users:
                    tx.insert(("has_role", user_value, "admin", Value("User", str(other_user.id))))
...
def sync_user_role_change(user: User, old_role: Optional[str] = None) -> bool:
    """
...
def sync_department_change(user: User, old_department: Optional[str] = None) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            if old_department and user.role == "nurse":
                old_patients = db.query(Patient).filter(
                    Patient.department == old_department,
                    Patient.is_active
                ).all()
